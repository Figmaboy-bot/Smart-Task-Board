    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session?.user) {
        if (mfaCheckedForUserId !== session.user.id) {
          ...
          // The callback runs inside supabase-js's internal auth lock, and
          // that lock isn't reentrant: awaiting another supabase.auth.* call
          // (checkMfaStatus) directly in here queues behind a lock the
          // callback itself is still holding, deadlocking until withTimeout
          // gives up. Deferring to the next tick lets the callback return
          // and release the lock first. ProtectedRoute re-renders
          // reactively once mfaRequired resolves, so the brief window
          // before it settles just delays the /verify-mfa redirect, it
          // doesn't bypass it.
